...
function create() {
    DiscordUtil.changePresence("VS Impostor CNE Port: In the Gallery", null);
    ...
    if (data.images[0] != null) {
        for (i in 0...data.images.length) addGalleryImage(data.images[i].filename, i);
        add(images);
    } else {
...
function update(elapsed:Float) {
	...
	images.forEach(function(img:FlxSprite){
		if (img.ID == curSelected) img.x = CoolUtil.fpsLerp(img.x, (FlxG.width - img.width) / 2, 0.125);
        else if (curSelected > img.ID) img.x = images.members[img.ID+1].x - images.members[img.ID].width - 100;
        else if (curSelected < img.ID) img.x = images.members[img.ID-1].x + images.members[img.ID-1].width + 100;
	});
}
...
inline function changeText():Void {
    ...
    title.x = (FlxG.width - title.width) / 2;
    FlxTween.tween(title, {y: images.members[curSelected].y / 2}, .35, {ease: FlxEase.quintOut});
